...
-- Run this instead of production_complete_schema.sql
-- ============================================================

USE `sunleaftechnologies`;

-- ============================================================
-- STEP 1: Drop old product_reviews table (data will be migrated)
-- ============================================================
-- Backup data first if needed
CREATE TABLE IF NOT EXISTS `product_reviews_backup` AS SELECT * FROM `product_reviews`;
...
-- TABLE: reviews (NEW - replaces product_reviews)
CREATE TABLE IF NOT EXISTS `reviews` (
  ...
  CONSTRAINT `fk_new_reviews_product`
    FOREIGN KEY (`product_id`) REFERENCES `products`(`id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_new_reviews_user`
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`)
    ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_new_reviews_order`
    FOREIGN KEY (`order_id`) REFERENCES `orders`(`id`)
    ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Migrate old review data
INSERT INTO `reviews` (`product_id`, `rating`, `comment`, `created_at`, `customer_name`, `status`)
SELECT `product_id`, `rating`, `comment`, `created_at`, 'Anonymous', 'approved'
FROM `product_reviews_backup`;
